...
NB: A row may correspond to more than 1 kernel when hierarchical effects like age, year, or site are included in the model. When this happens, `expand.grid()` is called on the list in `levels_hier_effs`, and all rows are `paste`'d together with `collapse = "_"`. This creates fully crossed levels. Each row that contains the hierarchical effects is then replicated, but with a single level substituted in for the suffix(es). These steps are implemented within `make_ipm()` (and, more specifically `.initialize_kernels()`), and so you will never actually see the results of the suffix expansion when you print `proto_ipm`, only the output from `make_ipm()`. 
