...
The only requirement for these functions is that they contain valid R code, and their return types are numeric.
...
`*_stoch_param` methods are, on the surface, slightly less computationally efficient than their `*_stoch_kern*` counterparts because they must rebuild the iteration kernel for every single iteration. However, they are fantastic tools for propagating uncertainty in parameter estimates throughout the model. In fact, the time they save on boot-strapping and re-building the vital rate models can make them even more efficient than their kernel-resampled counterparts. Below is an example that uses a multivariate normal distribution to re-generate parameter estimates for each iteration of the model. `ipmr` is careful to only evaluate each function only once per iteration, so you can use multivariate distributions without worrying about accidentally drawing too many times.
