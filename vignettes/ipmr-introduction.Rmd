...
"A density independent IPM", "A density dependent IPM", "A deterministic IPM", "A stochastic IPM", "If `stoch`, this indicates that parameters are resampled in each iteration and kernels are reconstructed with the new parameters", "If `stoch`, this indicates that kernels constructed from parameters ahead of time and then re-sampled at each iteration"))
...
`evict_cor` refers to whether or not to correct for eviction in the kernel. If this is set to `TRUE`, then you must supply a function specifying which expressions need to be corrected. `ipmr` provides `truncated_distributions` for now, though others will be implemented as well. 
...
- `env_list` contains the evaluation environments for each kernel and sub-kernel. These are used by subsequent analysis methods and so are important for internal usage, but are probably of limited use to most users. 
...
In this example, the random effects are not correlated with each other, corresponding to separate models for each vital rate. We will use the `purrr` package to make the first bit of code a bit more concise. This chunk takes the place of fitting regression models to actual data, so the code that replaces this chunk will look a little different (and probably involve the use of `coef(some_vital_rate_model`)).
...
The only requirement for these functions is that they contain valid R code, and their return types are numeric or integer.

### Initializing the IPM

With the functions and parameter values defined, we are now ready to begin composing the model. Each expression's syntax will look a little different from before, because we are appending a suffix corresponding to the hierarchical component of the model to each parameter/vital rate/kernel that is affected by it. In this case, we're using it as a year effect, denoted `_yr`, but any suffix will work. The first line creates a list with the suffix as the name and a vector corresponding to the levels it takes on. This can be a character or integer vector, and the list can have as many entries as required by the model (e.g. `yr` for years *and* `pl` for plots. The suffix would then become `_yr_pl`). The calls to `define_kernel` and `define_k` now have `has_hier_effs = TRUE`, and `levels_hier_effs = hier_levels`. Otherwise, the code should look quite familiar.
...
my_ipm <- init_ipm('simple_di_stoch_kern') %>%
  define_kernel(
    ...
    # params contains the named parameters g_r_1, g_r_2, s_r_1, s_r_2, etc.
...
In some cases, it is not desirable to work with point estimates of a random variable, and we prefer to work with the distributions they comes from. Quantifying uncertainty is a critical part of demography, and methods for propagating uncertainty in parameter estimates throughout a model are well established. Unfortunately, this also means that we can't obtain a set of kernels without iterating the model at least once. This is where the `*_stoch_param` methods come in handy.

`*_stoch_param` methods are, on the surface, slightly less computationally efficient than their `*_stoch_kern*` counterparts because they must rebuild the iteration kernel for every single iteration. However, they are fantastic tools for propagating uncertainty in parameter estimates throughout the model. Below is an example that uses a multivariate normal distribution to re-generate parameter estimates for each iteration of the model. `ipmr` is careful to only evaluate each function in `define_env_state()` only once per iteration, so you can use multivariate joint distributions correctly.

### Defining initial conditions

Stochastic simulations require specification of the initial conditions. `ipmr` aims to make this straightforward for you by providing two helpers - `define_pop_state()` and `define_env_state()`. Both are flexible in their syntax, though each does have a learning curve if you are not used to writing your own functions. The example below will use both when defining the model.

### `define_pop_state()`

`define_pop_state()` takes either a named set of expressions that, when evaluated, generate a population vector, or a named list of pre-generated population vectors in the `pop_vectors` argument. One important thing to note about working with population vectors in `ipmr` is that they should take the form `n_StateVariable_t` and `n_StateVariable_t_1`. `ipmr` uses these key words to make sure that population states are matched to the kernels that use them, and so the strict syntax in this area is needed to ensure accuracy in more complicated models. 

### `define_env_state()`

`define_env_state()` takes a named set of expressions in the `...` and then a data list, much like how `define_kernel()`/`define_k()` take them. The key difference here is that you will need to use the name on the left hand side of the `...` to access the values on the right hand side. In the example below, the function passed to `define_env_state()` returns a named list of length 4. However, we only want to access a single parameter in each vital rate expression. Since the list is named `env_params` and the variables we really want are named `s_int_yr`, `g_int_yr`, `f_r_int_yr`, `f_s_int_yr`, we access them using standard `$` syntax (e.g. `env_params$s_int_yr`) in each of the vital rate expressions.

Finally, this example uses the `mvtnorm` library to access the `rmvnorm` function for generating parameter estimates from a multivariate normal distribution during each model iteration.
...
r_sigma <- r_sigma %*% t(r_sigma) # make symmetrical - most users can/should skip this step.
...
# We define a wrapper function around rmvnorm to make sure
# that the names of the returned values match those in the vital rate
# expressions. Note that if you've already loaded mvtnorm via library(mvtnorm),
# you do not need the package::function() syntax. It is simply added here to 
# emphasize the rmvnorm does not come from ipmr.
...
# isn't strictly necessary, but makes the model code more readable.
...
param_resamp_model <- init_ipm('simple_di_stoch_param') %>%
  ...
  define_env_state(
    env_params = mvt_wrapper(r_means,
                             r_sigma, 
                             nms = c('s_int_yr',
                                     'g_int_yr',
                                     'f_r_int_yr',
