...
The first step of defining a model in `ipmr` is to initialize the model using `init_ipm()`. This function has two arguments: `model_class`, and `has_age`. We will ignore `has_age` for now, because age-size models are less common and have their [own vignette](https://levisc8.github.io/ipmr/articles/age_x_size.html). 

The `model_class` defines the type of projection model (deterministic or stochastic), and makes sure that the machinery for subsequent analyses works correctly. `model_class` should be a character string with at least 3 (but possibly 4) entries separated by underscores (`_`). The possible entries for each position are as follows:
...
With the type of model selected, the `model_class` becomes a string and the call to `init_ipm` is composed like so: `init_ipm(model_class = "position1_position2_position3_position4")`. 
