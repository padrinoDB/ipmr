...
my_ipm <- init_ipm(sim_gen = "simple", di_dd = "di", det_stoch = "det") %>%
  define_kernel(
    ...
    # Here, we tell ipmr that the model has some grouping variable, and 
  ...
  define_kernel(
    ...
    # As in the P kernel, we specify the levels the grouping variable
    # can take.
...
The example below is the same as the last example, except that this time, we call our grouping effect "year" (suffixed `_yr`) and we'll run a simulation by randomly choosing a year's kernel at each iteration ("kernel resampling" _sensu_ [Metcalf et al. 2015](https://besjournals.onlinelibrary.wiley.com/doi/pdf/10.1111/2041-210X.12405)). Along the way, we'll also learn how to pass custom functions to the model.
