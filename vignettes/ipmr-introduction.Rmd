...
The vast majority of modelling pipelines entail fitting a statistical models to data create vital rate functions. Thus, we're typically working with model objects rather than just parameter values. Sometimes the models have many terms, including interactions and/or hierarchical effects, making them annoying to write out in full. Furthermore, the flexibility of IPMs means our vital rate models may be semi- or fully non-parameteric, and writing out the functional form may not even be possible prior to the model fitting exercise (e.g. a GAM). 
...
3. Probability of flowering (`f_p`)
...
iteration_lambdas <- lambda(my_iterated_ipm, comp_method = 'pop_size', type = 'all')
