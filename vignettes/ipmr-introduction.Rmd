...
Say we have multiple sites sampled in the same year, but are only interested in their asymptotic dynamics (perhaps to compare fitness across space). We can use almost the exact same code as above - we switch the `model_class` argument of `init_ipm()` back to `simple_di_det` and set `make_ipm(iterate = FALSE)`. 
