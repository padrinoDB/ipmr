...
The `...` part of `define_env_state()` should be expressions that generate the variables you would like to reference. In this case `env_params` is a named list that contains `s_int`, `s_slope`, `g_int`, `g_slope`, and `g_sd`. The `s_int`, `s_slope`, etc are the names that will be generated by the IPM code, and so are the names that should be referenced in the kernels' vital rate expressions, not `env_params`! You don't need to remember that you called this particular object `env_params` when you write your vital rate expressions, but it does have to be named something for the IPM code to run. 
...
# If you want to see the actual draws that were used at each step of the 
# model iteration, you can access these using the output's $env_seq slot.
