...
2. Each kernel that has an integration requires that `d_statevariable` get appended to the kernel `formula`. This is equivalent to the "dz" in $\int_L^U K(z',z) n(z,t) \mathrm{dz}$. `ipmr` automatically generates this variable internally, so there is no need to define it in the `data_list` or `define_domains()`, you can just add it any of the formula(e) that you want to. We skipped this step in the simple IPMs because it gets appended internally. Unfortunately, it is decidedly less easy to infer the correct state variable to `d_z` with for general IPMs where there may be many continuous and/or discrete states.

  - NOTE: if in doubt, kernels that have a probability density function (e.g. `dnorm`, `dlnorm`, `dgamma`) in one of the underlying vital rates will pretty much always require this. Kernels that don't use these density functions may or may not require them. 
...
4. The implmentation arguments list can now have `NA`s in the `dom_start` and `dom_end` slots. This is demonstrated in a separate chunk below.
...
5. $leave\_discrete(z') = e_p * \int_L^Uf_d(z') \mathrm{dz}$
...
6. germination (`g_i`) and establishment (`e_p`) are constants. The code below assumes you have your data in long format (each seed get its own row) and that successful germinations/establishments are coded as 1s, failures are 0s, and seeds you don't know the fate of for whatever reason are `NA`s.
...
We've now defined all of the kernels, and have to define the implementation details. These also differ somewhat from simple IPMs. The key difference in the implementation arguments list lies in the `dom_start` and `dom_end` of each kernel, and is related to the `family` argument of each kernel. Continuous -> continuous transitions always have an entry for the starting and ending domains, but ones describing discrete transitions may have only one, or none. If the family is `"CD"`, then the `dom_start` is the name of the continuous domain, and the `dom_end` is `NA`. The opposite is true for `"DC"`. For `"DD"` transitions, both are `NA`. This reflects that there is no ending domain for a continuous -> discrete transition, no starting domain for a discrete -> continuous transition, and no domain at all for a discrete -> discrete transition. 
...
Our model is now built! We can explore the asymptotic population growth rate with the `lambda()` function, which will work on any object made my `make_ipm`. The same is true for `is_conv_to_asymptotic()`, which is a helper to function to figure out if you've set the number of `iterations` high enough to actually reach asymptotic population dynamics. 
...
general_stoch_kern_ipm <- init_ipm("general_di_stoch_kern") %>%
  ...
  make_ipm(iterations = 100,
           ...
           # This helps others to reproduce what you did (by setting a seed first),
           # and lets you keep track of the consequences of different selection
           # sequences for population dynamics. Note that you will need to specify
           # a seed and generate the kernel sequence as a variable
           # before building the IPM to make sure your output is fully reproducible!
...
We can also use continuously varying parameters to construct general IPMs. These are usually the result of multivariate and/or mixed effects models where joint parameter distributions are modeled, or environmental variation is included by sampling from environmental state distributions and using draws in vital rate expressions. Most Bayesian vital rate models would fall into this category as well. `ipmr` handles these using `define_env_state()`, which can take both functions and data and generate draws from distributions during each iteration of the model. 
...
# modify this to generate more realistic sampling probabilities.
...
general_stoch_param_model <- init_ipm("general_di_stoch_param") %>%
  define_kernel(
    ...
    # Every parameter in this particular kernel varies with the environment,
    # so we don't actually need to put the constant_params object into the
    # data_list. It never hurts though - the parameters in there are simply
    # ignored when the kernel is built.
  ...
  define_env_state(
    env_params = posterior_sample(posterior),
    data_list  = list(posterior = posterior_distribution)
  ) %>% 
  define_pop_state(
