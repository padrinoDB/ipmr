...
5. Recruit size distribution (`f_d`): A normal distribution with mean `f_d_mu` and standard deviation `f_d_sd`.
    ...
    - Mathematical form: $f_d(z') \sim Norm(\mu_{f_d}, \sigma_{f_d})$
...
Next, we set up two functions to pass into the model. These perform the inverse logit transformations for the probability of flowering model (`f_r`/$f_r(z)$) and survival model (`s`/$s(z)$).
...
eta_bar <- function(ipm) {
  ...
  N     <- make_N(ipm)
  e_fun <- matrix(1, nrow = 1, ncol = ncol(N))
...
We'll use a variation of the model above, simulating some random year-specific intercepts for growth and seed production. Parameters, functions, and kernels that are now time varying have a subscript, $yr$ appended to them.
...
4. $go\_discrete_{yr}(z) = f_{s,yr}(z) * f_r(z) * g_i$
...
2. growth (`g`): A linear mixed effects regression
    ...
    - Mathematical form: $mu_g = \alpha_g + \alpha_{g,yr} + \beta_g * z$, $g(z',z) \sim Norm(\mu_g, \sigma_g)$
...
4. seed production (`f_s`): A Poisson mixed effects regression
    ...
    - Mathematical form: $Log(f_s(z)) = \alpha_{f_s} + \alpha_{f_s,yr} + \beta_{f_s} * z$
...
A key aspect here is setting the names on the time-varying parameters in the `data_list`. We are using the form `"g_int_year"`, and substituting the values that `"year"` can take in for the actual suffix. These don't need to be numbers, they can be words, letters, or some combination of the two. When picking values, it is best to avoid longer words or phrases where words are separated by underscores (e.g. `treatment = c("h_removal", "c_addition"))`, as this may cause problems when values are substituted into expressions. CamelCase is generally safer in these cases (e.g. `treatment = c("hRemoval", "cAddition"))`. For this example, the years will be represented with the values `1:5`. If we wanted to denote the actual years of the censuses, we could switch `1:5` to, for example, `2002:2006`. 
...
general_stoch_kern_ipm <- init_ipm("general_di_stoch_kern") %>%
  define_kernel(
    ...
    # we also add the suffix here, because the paremter modified by truncated_distributions
    # is time-varying.
...
100 isn't too many iterations, but hopefully this demonstrates how to set up and implement such a model. There are a number of slots in the output that may be useful for further analyseses.
...
Additionally, there are functions to compute the stochastic population growth rate ($\lambda_s$, computed as `mean(log(pop_state$lambda))` with the first 10% of iterations removed as burn-in), and the mean sub-kernels.
...
1. $n(z', t+1) = \int_L^U K(z',z, \theta)n(z,t)dz + B(t) * r_s * r_d * \int_L^Uf_d(z')dz'$

2. $B(t + 1) = r_e * r_s * \int_L^U f_r(z) * f_s(z)n(z,t)dz + B(t) * r_s * r_r$

3. $K(z',z,\theta) = P(z',z,\theta) + F(z',z)$

4. $P(z',z,\theta) = s(z, \theta) * g(z',z,\theta)$

5. $F(z',z) = f_r(z) * f_s(z) * f_d(z') * (1 - r_e)$
...
`format_mega_matrix()` can also handle cases where you need blocks of 0s or identity matrices. These are specified using `0` for 0s, and `I` for identity matrices. `format_mega_matrix()` automatically works out the correct dimensions internally, so you don't need to worry about specifying those. 
...
mega_mat <- format_mega_matrix(general_ipm,
                               mega_mat = c(P, 0, 
                                            I, P))
...
Finally, `format_mega_matrix` supports `ipmr`'s suffix syntax as well, enabling us to generate lists of mega-kernels for each level of the grouping variables. We'll re-run the `"general_di_stoch_kern"` example from above to demonstrate this.
...
Next, we call `format_mega_matrix()` using the kernel names like so:

```{r eval = FALSE}

block_list <- format_mega_matrix(general_stoch_kern_ipm,
