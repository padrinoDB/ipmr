...
general_stoch_kern_ipm <- init_ipm("general_di_stoch_kern") %>%
  ...
  make_ipm(iterations = 100,
           ...
           # This helps others to reproduce what we did (by setting a seed first),
           # and lets us keep track of the consequences of different selection
           # sequences for population dynamics. Note that we will need to specify
           # a seed and generate the kernel sequence as a variable
           # before building the IPM to make sure our output is fully reproducible!
...
This example uses two Bayesian models for survival and growth and a single function to sample them to illustrate how to use continuously varying parameter distributions in `ipmr`. This only uses two models and one function to limit the the complexity of the example - there is no upper limit on the number of parameters or functions we can use in our own models. 
