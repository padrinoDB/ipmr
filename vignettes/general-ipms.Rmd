...
general_stoch_kern_ipm$env_seq
...
Sometimes, we may want to work with our sub-kernels arranged into a single block matrix. This isn't required for any of the code in `ipmr`, except for the `plot` method for `general_di_det` IPMs. Other use cases my arise for analyses not included in this package though, so below is a brief overview of how to generate those.
