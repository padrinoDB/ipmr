...
The $P(z', z, N)$ kernel is comprised of a density independent function for growth (Eq 6-7) and a density dependent function for survival (Eq 5). $f_g$ denotes a Gaussian probability density function: 

4. $P(z', z, N) = s(z, N) * g(z', z)$

5. $Logit(s(z, N)) = \alpha_s + \alpha_s^{yr} + \beta_s^z * z + \beta_s^{N} * N$

6. $g(z', z, \theta) = f_g(z', \mu_g(z, \theta), \sigma_g)$

7. $\mu_g(z, \theta) = \alpha_g + \alpha_g^{yr} + \beta_g^z * z$

The $F(z',z, N, \theta)$ kernel is comprised of a density independent function for recruit size (Eq 10) and probability of reproducing (Eq 9), and a density dependent function for number of recruits produced by parents (Eq 11). $f_{r_d}$ denotes a Gaussian probability density function: 

8. $F(z', z, N, \theta) = r_r(z) * r_s(z, N, \theta) + r_d(z')$

9. $Logit(r_r(z)) = \alpha_{r_r} + \beta_{r_r}^z * z$

10. $r_d(z') = f_{r_d}(z', \mu_{r_d}, \sigma_{r_d})$

11. $Log(r_s(z, N, \theta)) = \alpha_{r_s} + \alpha_{r_s}^{yr} + \beta_{r_s}^z * z + \beta_{r_s}^N * N$
...
dd_ipm <- define_kernel(
  ...
  r_s_yr           = exp(r_s_int + r_s_r_yr + r_s_slope * size_1 + r_s_dd * sum(n_size_t) * d_size),
