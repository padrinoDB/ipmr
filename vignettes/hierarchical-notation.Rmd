...
ex_ipm <- init_ipm("general_di_stoch_param") %>%
  define_kernel(
    ...
    s_site_lin       = alpha_s_site + beta_s_site * z_1 + theta_s,
    s_site           = 1 / (1 + exp(-s_site_lin)),
    mu_g_site        = alpha_g_site + beta_g_site * z_1 + theta_g,
  ...
  define_kernel(
    ...
    f_p_site_lin     = alpha_f_p_site + beta_f_p_site * z_1 + theta_f_p,
    f_p_site         = 1 / (1 + exp(-f_p_site_lin)),
    f_r_site         = exp(alpha_f_r_site + beta_f_r_site * z_1 + theta_f_p),
    f_d              = dnorm(z_2, mu_f_d, sigma_f_d),
    data_list        = all_pars,
    states           = list(c("z")),
    has_hier_effs    = TRUE,
  ...
  make_ipm(
    usr_funs           = list(theta_fun = theta_fun),
    iterations         = 200,
    kernel_seq         = sample(LETTERS[1:6], 200, replace = TRUE),
    normalize_pop_size = TRUE
  )
