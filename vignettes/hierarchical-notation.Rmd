...
6. $logit(s^{site}) = \alpha_{s}^{site} + \beta_s^{site} * z$

7. $g^{site} \sim Norm(\mu_g^{site}, \sigma_g^{site})$

8. $\mu_g^{site} = \alpha_g^{site} + \beta_g^{site} * z$

9. $logit(f_{p}^{site}) = \alpha_{f_p}^{site} + \beta_{f_p}^{site} * z$

10. $log(f_{r}^{site}) = \alpha_{f_r}^{site} + \beta_{f_r}^{site} * z$

11. $f_d \sim Norm(\mu_{f_d}, \sigma_{f_d})$

### Simulating parameters

We have to simulate a posterior distribution for the example code to work. Normally, model fitting using actual data would fill the space that this is taking up.

We'll use 6 sites for the discrete portion of our variables. There's some initial set up for each one, and then we simulate a posterior distribution using various random number generators.
...
# Generate names for all unique parameters
all_ran_pars <- expand.grid(pars = pars, vrs = vrs, sites = sites)

all_ran_nms <- paste(all_ran_pars$pars, 
                     all_ran_pars$vrs,
                     all_ran_pars$sites, 
                     sep = "_")

all_ran_nms <- c(all_ran_nms, paste('sigma_g_', sites, sep = ""))

# This loop will generate the "posterior samples" for the model. 

post_distrib <- vector(mode = "list", length = length(all_ran_nms))
post_distrib <- lapply(post_distrib,
                       function(x, ll) {
                         out <- numeric(ll)
                         return(out)
                       },
...
for(i in seq_along(sites)) {
  ...
  # index for the g_sigma parameter because it was tacked on at the end
  # to avoid expand.grid'ing a sigma into every site X vital rate combination
  
  g_sig_ind <- length(all_ran_nms) - 6 + i
  
  # draws for s function parameters
  
  s_int <- rnorm(4000, mean = -9, sd = 1)
  s_slo <- runif(4000, min = 1, max = 2.5)
  
  # draws for g function parameters
  
  g_int <- rnorm(4000, mean = 0, sd = 0.3)
  g_slo <- runif(4000, 0.85, 1.15)
  g_sig <- runif(4000, 0.2, 1.5)
  
  # draws for f_p function parameters
  
  f_p_int <- rnorm(4000, mean = -2, sd = 0.3)
  f_p_slo <- runif(4000, 1, 1.9)
  
  # draws for f_r function parameters
  
  f_r_int <- rgamma(4000, shape = 1)
  f_r_slo <- runif(4000, 0.001, 0.1)
  
  
  post_distrib[[start]] <- s_int
  post_distrib[[(start + 1)]] <- s_slo
  post_distrib[[(start + 2)]] <- g_int
  post_distrib[[(start + 3)]] <- g_slo
  post_distrib[[(start + 4)]] <- f_p_int
  post_distrib[[(start + 5)]] <- f_p_slo
  post_distrib[[(start + 6)]] <- f_r_int
  post_distrib[[(start + 7)]] <- f_r_slo
  post_distrib[[g_sig_ind]]   <- g_sig
  
 }


names(post_distrib) <- all_ran_nms
post_distrib        <- as.data.frame(post_distrib)
...
Additionally, we need to write a function that samples from the posterior distributions and creates variables with the right names.  

```{r eval = FALSE}

post_fun <- function(post_distrib) {
  
  post_size  <- dim(post_distrib)[1]
  
  sample_ind <- sample(seq_len(post_size),
                       size = 1)
  
  out        <- as.list(post_distrib[sample_ind, ])
  
  names(out) <- names(post_distrib)
...
ex_ipm <- init_ipm("general_di_stoch_param") %>%
  define_kernel(
    ...
    s_site_lin       = alpha_s_site + beta_s_site * z_1,
    s_site           = 1 / (1 + exp(-s_site_lin)),
    mu_g_site        = alpha_g_site + beta_g_site * z_1,
    g_site           = dnorm(z_2, mu_g_site, sigma_g_site),
    data_list        = all_pars,
    states           = list(c("z")),
    has_hier_effs    = TRUE,
  ...
  define_kernel(
    ...
    f_p_site_lin     = alpha_f_p_site + beta_f_p_site * z_1,
    f_p_site         = 1 / (1 + exp(-f_p_site_lin)),
    f_r_site         = exp(alpha_f_r_site + beta_f_r_site * z_1),
                           f_d              = dnorm(z_2, mu_f_d, sigma_f_d),
                           data_list        = all_pars,
                           states           = list(c("z")),
                           has_hier_effs    = TRUE,
      ...
      define_env_state(
        draws    = post_fun(posterior),
        data_list = list(posterior = post_distrib)
      ) %>%  
      make_ipm(
        usr_funs           = list(post_fun = post_fun),
        iterations         = 20,
