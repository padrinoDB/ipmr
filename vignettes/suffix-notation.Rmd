---
title: "Suffix Notation in `ipmr`"
output:
  rmarkdown::html_vignette: 
    toc: yes
    toc_depth: 3
vignette: >
  %\VignetteIndexEntry{Suffix Notation}
...
6. Probability of reproducing
    ...
    - $Logit(r_p) = \alpha_{r_p} + \beta_{r_p} * z$
    
7. Recruit production

    - $log(r_r) = \alpha_{r_r} + \beta_{r_r} * z$
...
We are now ready to begin implementing the kernels. This model will be a simple, density independent, stochastic kernel-resampled model. The parts where the suffix syntax is used are highlighted by comments in the code block:
...
ex_ipm <-init_ipm(sim_gen    = "simple",
  ...
  define_kernel(
    ...
    # We signal that the kernel has grouping effects
...
9. $g^{site}(z',z,\theta) = f_g(\mu_g^{site}(z, \theta), \sigma_g)$
...
13. $c_d(z') = f_{c_d}(\mu_{c_d}, \sigma_{c_d})$
