...
The expression for `n_wt_0_t_1` uses `sum` to reflect that we want to use all ages in our `levels_ages` list to generate a single trait distribution for age-0 individuals. This is slightly different from the usual suffix expansion in that rather than generating many separate expressions, it generates a single expression that is a lot longer than the short-hand we provide. 
