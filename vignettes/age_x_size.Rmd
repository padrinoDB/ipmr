...
Our call to `define_k` will look slightly different. We will have to specify the expressions that produce the age-0 individuals and the expressions produce age > 0 individuals separately. Additionally, we need to specify a third expression that generates the `"max_age"` individals. 
...
The helper function `all_ages` is a convenience function that helps us say "all ages should be included in this single expression". In other words, we do not need to write `n_wt_0_t_1 = F_0 %*% n_wt_0_t + F_1 %*% n_wt_1_t + ...`. The `expr` argument takes the generic form of the expression, and the `fun` argument takes a function that collapses the result into a single vector. The `fun` argument will almost always be `"+"`, as we wish to add together all newborns produced by every older age class. Note that `fun` needs to be a string (i.e. `"+"`, not `` `+` ``) - supplying back-ticked symbols will result in an error.

Next, we define a generic expression for the size distribution of each 0 < age < max_age. This is the survival/growth kernel for individuals `age - 1` times the size distribution of individals at `age - 1`. Unfortunately, we can't insert literal `-` symbols in our expressions (R's parser does not like this!). Instead, we use `_minus_` and `_plus_` as needed. We are not restricted to `_minus_1` or `_plus_1` - models that have longer time lags can be denoted `_minus_2`, `_plus_2`, etc.

Finally, we need an expression for the maximum aged individuals. This is all surviving ones from the previous year, plus all the ones that were `max_age - 1` that survived. 
...
4. Optionally, `define_env_state()` (though not here because it's a deterministic model)
