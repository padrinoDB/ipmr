...
# Now, we'll see how long it takes when the complete time series gets
# stored

n_t <- lapply(1:22,
              function(x) {
                matrix(0, ncol = 101, nrow = 100)
              })

n_t[[1]][ , 1] <- with(i.par, rep(1 / m, m))

hand_time_2 <- system.time({

  IPM.sys <- mk_age_IPM(i.par, m.par.true)
  x   <- lapply(n_t, function(x) x[ , 1])
  lam <- rep(NA_real_, 100)

  for(i in seq_len(100)) {

    x1     <- r_iter(x, IPM.sys$na, IPM.sys$F, IPM.sys$P)
    lam[i] <- sum(unlist(x1))
    x      <- lapply(x1, function(x, iter) x / lam[iter], iter = i)

    n_t <- lapply(1:22, function(y, holder, current, iter) {

      holder[[y]][ , (iter + 1)] <- current[[y]]
