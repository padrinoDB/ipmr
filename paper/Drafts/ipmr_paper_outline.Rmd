...
1. Integral projection models (IPMs) have become an important tool for studying the demography of populations structured by one or more continuous state variables (e.g. size, height, color). IPMs are used to investigate questions ranging from single species conservation and management strategies for invasive species to understanding selective pressures in natural populations. _R_ is a commonly used data analysis tool for ecologists. Previous _R_ package developments in structured population models introduced a broad repetoire of functionality and outputs (e.g. IPMpack). However, pressing ecological, evolutionary, and conservation biology topics require developing more realistic IPMs. Here, we introduce `ipmr`, a programmatic solution to flexibly build, analyze, and interpret IPMs.
...
4. `ipmr` is a flexible _R_ package for integral projection models. The package can substantially reduce the amount of time required to implement simple and complicated IPMs. We also provide extensive documentation with 8 vignettes and help files, accessible from an R session and online.
...
Integral projection models (IPMs) are an important and widely used tool for ecologists studying population dynamics in discrete time. Since their introduction over two decades ago (Easterling, Ellner & Dixon 2000),over 270 peer-reviewed publications have used IPMs to address questions ranging from invasive species population dynamics (e.g. Crandall & Knight 2017) to evolutionary stable strategies to endangered species conservation (Childs et al. 2011, Coulson 2012). There are currently at least 300 plant species and 60 animal species for which an IPM exists (Levin et al. unpublished). Some of the advantages of using an IPM include (i) continuously distributed state variables to describe vital rates (e.g. survival, fecundity) and population structure, (ii) the ability to mix discrete and continuous states in the same model (e.g. number of seeds in a seedbank and the dynamics of a height structured plant population, or number resting _Daphnia_ eggs produced by size-structured _Daphnia_ individuals), (iii) efficient parameter estimation with widely used regression methods, (iv) the numerical discretization of continuous kernels (see below) means that many tools available for matrix models are also available for IPMs, and (v) examining life history traits at more detailed levels (e.g. the kernel/sub-kernel level, the vital rate function level, and regression coefficient level). Furthermore, in recent years, researchers have developed methods to incorporate spatial dynamics (Jongejans et al. 2011), environmental stochasticity (Rees & Ellner 2009), and density/frequency dependence into IPMs (Adler et al. 2010, Ellner et al. 2016).

IPMs are flexible models that typically use longitudinal data to determine the functional forms of state-fate relationships such as survival, change in state (e.g. growth/shrinkage, development), and (a)sexual reproduction. IPMs are built using regression models of vital rates on a state variable(s) that serve as proxies for individual fitness. State variables are usually related to size (e.g. DBH, Metcalf et al. 2009, Crandall & Knight 2017, body mass for animals, Ozgul et al. 2010, combination of body weight and fur color in Coulson et al. 2011, combination of hatch date and weather conditions in Simmonds et al. 2020). Regression modelling is both a standard and highly flexible tool for ecologists, and the field is mature enough to accomodate many different types of data (e.g. Wood 2011, Bates et al. 2015). A particularly pertinent point for those interested in demography is that parameter estimation for regression models is usually more efficient than estimating individual matrix transition parameters (Ellner & Rees 2006, Ramula et al. 2009). There are usually far fewer parameters to estimate with the same amount of data (in theory, one could estimate matrix transitions with regression models, but this is not often done in practice). This flexibility in regression parameterization, and by extension, IPM parameterization, has lead to a rapid increase in publications using IPMs (Figure 1). This rise in popularity spurred the creation of software tools and guides that assisted in their implementation and analysis (Coulson 2012, Metcalf et al. 2013, Merow et al. 2014, Ellner et al. 2016). These tools range from _R_ scripts with detailed annotations to a complete R package.
