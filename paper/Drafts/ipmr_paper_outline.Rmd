...
Eq 9 is a recruit size distribution (where $f_{r_d}$ denotes a normal probability density function), and Eq 10 describes the number of new recruits produced by adults as a function of size $z$.

```{r eval = FALSE}

library(ipmr)

# This section takes the place of Step 1 in Figure 1.
...
The code in Box one substitutes the actual probability density function (`dnorm()`) for the $f_G$ and $f_{r_d}$, and uses inverse link functions instead of link functions. Otherwise, the math and the code should look quite similar. 
