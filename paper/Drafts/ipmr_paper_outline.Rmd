...
Perhaps the most frequently used metric derived from IPMs is the per-capita growth rate of the population once it has passed through its transient phase ($\lambda$). When $\lambda > 1$, the population is growing, while $\lambda < 1$ indicates population decline. `ipmr` makes deriving estimates of $\lambda$ straightforward. _Oenothera glazioviana_ is a monocarpic perennial that grows in sand dunes. Data were collected by Kachi & Hirose (1985) in sand dunes in Japan for 5 years. Ellner, Childs & Rees (2016), Chapter 2 used this data to parameterize a simple, deterministic IPM and estimate $\lambda$. We re-build the Ellner, Childs, & Rees (2016) model to illustrate how to go from parameter estimates to $\lambda$ in a few steps. 
...
monocarp_proto_ipm <- init_ipm(sim_gen = "simple", di_dd = "di", det_stoch = "det") 

monocarp_proto_ipm <- define_kernel(
    monocarp_proto_ipm,
    name         = "P",
    formula      = (1 - flower_prob) * surv * Grow,
    flower_prob  = plogis(f_i + f_z  * z_1), 
...
monocarp_proto_ipm <- define_kernel(
    monocarp_proto_ipm,
    name        = "F",
    formula     = flower_prob * seed_number * recr_size * e_p,
    flower_prob = plogis(f_i + f_z  * z_1),
    seed_number = exp(p_i + p_z * z_1),
    recr_size   = dnorm(z_2, mu_r, sd_r),
    data_list   = data_list,
    states      = list(c("z"))
  )

monocarp_proto_ipm <- define_impl(
    monocarp_proto_ipm,
    list(
...
monocarp_proto_ipm <- define_domains(
  monocarp_proto_ipm,
  z = c(-2.65, 4.5, 250) # format: c(L, U, m), m is number of meshpoints
)

monocarp_proto_ipm <- define_pop_state(
  monocarp_proto_ipm,
  n_z = rep(1/250, 250)
) 

monocarp_ipm <- make_ipm(monocarp_ipm)

lambda(monocarp_ipm)

```

*Box 1: Code to implement a simple IPM from parameter estimates in `ipmr`. Because `ipmr` does not include functions to assist with regression modeling, this example skips the step of working with actual data and instead takes parameter values that the authors provide. We see that given this set of conditions, if nothing were to change, the population would increase by ~6% each year. The case studies provide details on further use cases and analyses that are possible with `ipmr`.*
...
21. Kachi, H., & Hirose, T. (1985). Population dynamics of _Oenothera glazioviana_ in a sand-dune system with special reference to the adaptive significance of size-dependent reproduction. Journal of Ecology 73: 887-901. https://doi.org/10.2307/2260155   
