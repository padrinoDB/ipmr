...
Integral projection models (IPMs) are an important tool for ecologists studying population dynamics in discrete time. Since their introduction in Easterling, Ellner & Dixon's (2000) paper, there have been over 270 publications using IPMs to address a variety of questions ranging from invasive species population dynamics (e.g. Crandall & Knight 2017) to evolutionary stable strategies to endangered species conservation (Childs et al. 2011, Coulson 2012). In the plant kingdom alone, there are currently at least 300 plant species for which an IPM exists (Levin et al. unpublished). Advantages of using an IPM include (i) they allow for continuously distributed state variables to describe vital rates and population structure, (ii) the ability to mix discrete and continuous states in the same population model, and (iii) allow for efficient parameter estimation with widely used regression methods. Since the first IPM publication, methods to environmental stochasticity, spatial dynamics, and density dependence have been developed (Ellner & Rees 2006, Adler et al. 2010, Ellner et al. 2016).

IPMs are flexible models that use longitudinal data to determine the functional forms of state-fate relationships such as survival, growth/maturation, and (a)sexual reproduction. They are built using regression models of vital rates on a state variable that is related to individual fitness (usually, but not always, a measure of size, e.g. diameter at breast height for trees, body mass in animals). Regression modelling is a standard tool for ecologists, and the field is mature enough to accomodate many different types of data (e.g. Wood 2011, Bates et al. 2015). Parameter estimation for regression models is usually more efficient than estimating individual matrix transition parameters (Ellner & Rees 2006); there are usually fewer parameters to estimate with the same amount of data (in theory, one could estimate matrix transitions with regression models, but this is not often done in practice). This flexibility has lead to a rapid increase in their appearance in the literature (Figure 1). Additionally, this rise in popularity spurred the creation of software tools and guides that assisted in their implementation and analysis (Metcalf et al. 2013, Merow et al. 2014, Ellner et al. 2016). These range from _R_ scripts with detailed annotations to a complete R package.
