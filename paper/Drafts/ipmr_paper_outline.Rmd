...
Here, $n(z, t)$ and $n(z',t+1)$ are the distribution functions for a trait $Z$ at time $t$ and $t+1$. $L,U$ are the lower and upper bounds of values that the trait $Z$ can have, and $K(z',z)$ is the projection kernel. The integral of $n(z,t)$ over $L,U$ is the total population size. To make the model more biologically interpretable, the $K(z',z)$ kernel is usually decomposed into sub-kernels. For example, a model that captures survival and maturation transitions and sexual reproduction may be written as follows:

$$K(z',z) = P(z',z) + F(z',z) + C(z',z)$$ {#eq:eqn2}

Here, $P(z',z)$ describes transitions due to survival and state values changes of existing individuals, $F(z',z)$ describes transitions that create new individuals through sexual reproduction, and $C(z',z)$ to describe transtions that create individuals through asexual reproduction. The sub-kernels themselves are typically generated with functions parameterized by regression models that relate the individuals trait value $z$ at time $t$ to a new trait value at $t+1$. For example, the $P$ kernel for Soay sheep on St. Kilda may contain two regression models: (i) a logistic regression of survival on log body mass (Eq 4), and (ii) a regression of log body mass at $t+1$ on log body mass at $t$ (Eq 5-6). In this example, $\sigma_g$ is the standard deviation of the residuals from the growth regression model.
...
We refer to these different scales (parameters, regressions, sub-kernels, and projection kernels) as levels of the model hierarchy. There have been numerous extensions of IPM theory. Examples include the general, deterministic IPM that allows for a mixture of multiple discrete and/or continuous state variables (Ellner & Rees 2006), stochastically varying parameters to generate unique kernels (Childs et al. 2004, Rees & Ellner 2009), and spatial dynamics (Jongejans et al. 2011).

Analytical solutions to the integrals in Equation 1 are usually too complicated (or not possible; Ellner & Rees 2006). Numerical approximations of these integrals are typically quite easy to compute though. Examples of integration rules are the midpoint rule and the trapezoid rule for integration, though more complicated and precise methods have also been used (Zuidema et al. 2010). A matrix multiplication of the discretized projection kernel and the discretized trait distribution then generates a new trait distribution, a process referred to as model iteration (sensu Easterling et al. 2000). 

A critical aspect of `ipmr`'s functionality is the distinction between simple and general IPMs. A simple model is any IPM that makes use of a single continuous state variable, and models no additional structure in the population (i.e. contains no discrete states). Equations 1 and 2 represent a simple IPM. A general IPM models one or more continuous state variables, and/or discrete states. General IPMs are useful for modelling species with more complex demography than can be described by a single, continuous state variable. In reality, many species may have multiple life stages that are not readily described by a single state variable, and/or individuals with similar trait values may behave differently depending on environmental context. For example, Bruno et al. (2011) modeled aspergillosis impacts on sea fan population dynamics by creating a model where colonies were cross classified by tissue area (continuously distributed) and infection status (a discrete state with two levels - infected and uninfected). A simple IPM could not capture this level of detail. A more comprehensive definition of the simple/general distinction is given in Ellner, Childs & Rees (2016, Chapter 6).

# Design and usage

The `ipmr` model construction syntax is designed to mimic the mathematical notation of the IPM (Tables 1 & 2). The package is powered by the `rlang` toolkit for metaprogramming (Henry & Wickham 2019). Metaprogramming views code as data, enabling programmers to capture, inspect, and modify user code before evaluating it. This allows implementation of miniature, domain specific languages (Wickham 2020). On the user side, the package works by building up expressions that reference each other at higher and higher levels in the model hierarchy (i.e. regression models, sub-kernels, projection kernels). `ipmr` is relatively dependency-free, requiring only `rlang`, `purrr` (Henry & Wickham 2020), and `magrittr` (Bache & Wickham 2014). The model definition functions (`define_kernel`, `define_k`) can handle any valid R code in the expressions that are passed to them, so user-specified functions, in addition to ones included in other packages can all be used in vital rate and kernel expressions (e.g. `predict.*` methods for complicated vital rate models). Additionally, there are helper functions that define the kernel implementation, the initial state of the system, and optionally, how the system changes over the course of a simulation (e.g. environmental fluctuations and/or correlations). Finally, the package implements a suffix-based syntax for symbolically representing models with one or more grouping variables (e.g. data from multiple sites, years, and/or an age-structured model, see Table 2 and code in the ESM for details). All functions, methods, and classes are documented extensively on the package's website (https://levisc8.github.io/ipmr/), and in the package's vignettes and help files. Figure 2 shows a generic IPM workflow from collecting the raw data to biological inference, highlighting the stages at which `ipmr` is useful. 
...
Currently, time-lagged models are not possible in `ipmr` (e.g. Kuss et al. 2008). These are slated for development, and should be available in the next year or so. Additionally, while `ipmr` does handle the implementation step of IPMs and some very basic analyses (e.g. per-capita growth rates, left and right eigenvectors), this is rarely the end of the analysis for researchers. While we have demonstrated how to do some further analyses in the case studies, they are hardly exhaustive. We are developing a separate  R package to do more complicated analyses with IPMs (e.g. life table response experiments, life history trait analysis). This package is designed to work with `ipmr`'s class structure, and, once stable, will provide a complete tool set for analysis of populations structured by continuously distributed traits.
...
21. Henry L. and Wickham H. (2020). rlang: Functions for Base Types and Core R and 'Tidyverse'  Features. R package version 0.4.7. https://CRAN.R-project.org/package=rlang
...
35. Simmonds EG, Cole E.F., Sheldon B.C., & Coulson T.N. (2020). Phenological asynchrony: a ticking time-bomb for seemingly stable populations? Ecology Letters. https://doi.org/10.1111/ele/13603
