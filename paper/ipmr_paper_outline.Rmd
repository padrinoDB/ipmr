...
grViz("digraph flowchart {
      ...
      [1]: 'Fit vital rate models using other packages (e.g. lme4, nlme, mgcv)'
      [2]: 'Move coefficients or model objects into a named list'
      [3]: 'Decide whether the model is stochastic or deterministic and init_ipm() using the appropriate class'
      [4]: 'Build expressions for the individual kernels (e.g. P = s_g_mult(s, g)). When models are implemented from hierarchical vital rate models, take advantage of built-in suffix recognition in ipmr! (e.g. s_g_mult(s_yr, g_yr))'
      [5]: 'Build expressions for the individual vital rates using mathematical syntax, R functions, or some combination thereof (e.g. s = inv_logit(s_int, s_slope, sv_1))' 
      [6]: 'Generate an implementation argument list, define domains, and make_ipm()'
...
knitr::kable(
  tibble::tribble(
    ...
    "Prospective perturbation analyses (kernel level)",          "X",       "X", 
    "Prospective perturbation analyses (coefficient level)",     "X",       "X", 
    "Prospective perturbation analyses (vital rate level)",      "X",      "No", 
    "Implement discrete/continuous combinations",                "X",       "X",
    "Basic implementation diagnostics",                          "X",       "X",
    "Age-specific demographic functions",                        "X",       "X",
    ...
    "Life table respose experiments",                            "X",      "No",
    "Density dependent models",                                  "X",      "No",
