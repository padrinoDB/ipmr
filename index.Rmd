...
Once all parameters are estimated, the first step of defining a model in `ipmr` is to initialize the model using `init_ipm()`. This function has two arguments: `model_class`, and `has_age`. We will ignore `has_age` for now, because age $\times$ size models are a complicated topic and have their [own vignette](https://levisc8.github.io/ipmr/articles/age_x_size.html).
